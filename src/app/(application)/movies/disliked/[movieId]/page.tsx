...
import { RemoveDislikeButton } from "~/app/_components/movies/remove_dislike";


export default async function DislikedMoviesScrollView({ params }: { params: Promise<{ movieId: string }> }) {
    ...
    const movieIndex = dislikedMovies.findIndex((m) => m.movieId === movieId);

    const prev = dislikedMovies[movieIndex - 1];
    const next = dislikedMovies[movieIndex + 1];
            ...
            <div className="flex flex-wrap w-11/12 gap-6 justify-center pb-28 max-h-[31rem] overflow-y-auto">
                ...
                <div className="flex flex-row gap-4 items-center">
                    ...
                    <div className="flex flex-col items-start gap-4 w-24">
                        <RemoveDislikeButton movieId={movieId} nextId={next?.movieId} prevId={prev?.movieId} />
                    </div>
                    ...
                    <div className="flex flex-col items-center justify-center gap-4 w-24">
                        {prev ? (
                            <Link href={`/movies/disliked/${prev.movieId}`}>
                        ...
                        {next ? (
                            <Link href={`/movies/disliked/${next.movieId}`}>
                                <button className="group active:scale-90 size-10 flex items-center justify-center rounded-full transition-all bg-neutral-800/70 hover:bg-neutral-900/90 border border-neutral-300/20 hover:cursor-pointer backdrop-blur-md shadow">
